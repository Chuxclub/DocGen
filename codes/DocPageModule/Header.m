...
classdef (Abstract) Header
    methods (Static)
        function makeHeader(fid, title, longTitle, authors, contact)
            ...
            fprintf(fid, ['<p style="margin: 0 0 10px; padding: 0"> Documentation gÈnÈrÈe le <span style="font-size: 14px; font-weight: bold;">' datestr(datetime('today')) '</span> par DocGen</p>']);
            fprintf(fid, ' </div>');
            fprintf(fid, ' </html>\n');
        end
